...
<script>
    export default {
        ...
        methods: {
            handleSubmit() {
                if (this.submitMethod === "create") {
                    ...
                    axios.put(this.$baseURL + 'library/departments/' + this.library.id, this.library)
                        .then(response => {
                            this.$notification.success(response.data.message);
                            this.onClose();
                        })
        ...
        mounted: function () {
            ...
            this.$eventBus.$on("edit-department", department => {
                this.submitMethod = "update";
                this.library = department;
        ...
        beforeDestroy() {
            this.$eventBus.$off("edit-department");
        }
    };
</script>
