...
<script>
    export default {
            ...
            handleSubmit() {
                if (this.submitMethod === "create") {
                    axios.post(this.$baseURL + 'library/sections', this.library).then(response => {
                        this.$eventBus.$emit('add-section', response.data.data);
        ...
        mounted: function () {
            ...
            this.$eventBus.$on("edit-section", library => {
                this.submitMethod = "update";
                this.library = library;
                this.$emit("update:dialogVisible", true)
                    .$emit("update:dialogTitle", "Section update");
            });
        },
        computed: {
            //
        ...
        beforeDestroy() {
            this.$eventBus.$off("edit-section");
        }
    };
</script>
