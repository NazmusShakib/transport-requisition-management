<template>
    <div class="container-fluid">
        ...
        <!-- /.row -->
        <division-modal :dialog-visible.sync="dialogVisible" :dialog-title.sync="dialogTitle"/>
    </div>
</template>
...
<script>
    ...
    import DivisionModal from "~/components/library/division/_DivisionModalComponent";
    ...
    export default {
        ...
        methods: {
            getLibraries() {
                axios.get(this.$baseURL + "library/divisions?page=" + this.libraries.current_page)
                    .then(response => {
            ...
            destroy(id, index) {
                MessageBox.confirm(
                    ...
                    .then(() => {
                        axios.delete(this.$baseURL + "library/divisions/" + id)
                            .then(response => {
            ...
            edit(library, index) {
                this.$eventBus.$emit('edit-division', library);
            }
        },
        created() {
            this.$emit("update:layout", MasterLayout);
            this.$eventBus.$on("add-division", library => {
                this.libraries.total++;
