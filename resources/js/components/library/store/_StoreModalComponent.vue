...
<script>
    export default {
        ...
        methods: {
            handleSubmit() {
                if (this.submitMethod === "create") {
                    axios.post(this.$baseURL + 'library/stores', this.library).then(response => {
                        this.$eventBus.$emit('add-store', response.data.data);
                    ...
                    axios.put(this.$baseURL + 'library/stores/' + this.library.id, this.library)
                        .then(response => {
                            this.$notification.success(response.data.message);
                            this.onClose();
                        })
        ...
        mounted: function () {
            ...
            this.$eventBus.$on("edit-store", library => {
                this.submitMethod = "update";
                this.library = library;
                this.$emit("update:dialogVisible", true)
                    .$emit("update:dialogTitle", "Store update");
            });
        },
        computed: {
            //
