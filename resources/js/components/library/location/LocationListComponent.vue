<template>
    <div class="container-fluid">
        ...
        <!-- /.row -->
        <location-modal :dialog-visible.sync="dialogVisible" :dialog-title.sync="dialogTitle"/>
    </div>
</template>
        ...
        methods: {
            ...
            destroy(id, index) {
                MessageBox.confirm(
                    ...
                    .then(() => {
                        axios.delete(this.$baseURL + "library/locations/" + id)
                            .then(response => {
            ...
            edit(library, index) {
                this.$eventBus.$emit('edit-location', library);
            }
        },
        created() {
            this.$emit("update:layout", MasterLayout);
            this.$eventBus.$on("add-location", library => {
                this.libraries.total++;
