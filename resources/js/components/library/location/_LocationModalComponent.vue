...
<script>
    export default {
        ...
        methods: {
            handleSubmit() {
                if (this.submitMethod === "create") {
                    axios.post(this.$baseURL + 'library/locations', this.library).then(response => {
                        this.$eventBus.$emit('add-location', response.data.data);
                    ...
                    axios.put(this.$baseURL + 'library/locations/' + this.library.id, this.library)
                        .then(response => {
                            this.$notification.success(response.data.message);
                            this.onClose();
                        })
        ...
        beforeDestroy() {
            this.$eventBus.$off("edit-location");
        }
    };
</script>
